...
# ('-L', r'Lyrics:\s*(.+)'), # -lyrics      NUM  Set the lyrics # multiline
  ...
  ('-m', r'\s*Description:\s*(.{1,250}).*'), # -description STR  Set the short description
...
def mp4meta_import(f):
  ...
  if args.loglevel <= logging.DEBUG: log.debug('After tags remove info:\n' + subprocess.check_output(['mp4info', f]).decode(encoding='cp1252'))
  if ts:
  ...
  if args.loglevel <= logging.DEBUG: log.debug('After art remove info:\n' + subprocess.check_output(['mp4info', f]).decode(encoding='cp1252'))
  cs = reglob(re.escape(b)+r'(\.cover)?(\.art\[\d+\])?\.(jpg|png|gif|bmp)')
  if cs:
    call  = ['mp4art']
  ...
  if args.loglevel <= logging.DEBUG: log.debug('Final info:\n' + subprocess.check_output(['mp4info', f]).decode(encoding='cp1252'))
...
if __name__ == '__main__':
  ...
  parser = argparse.ArgumentParser(description="Export, import, and swap mp4 meta data.")
  ...
  if args.operation=='export':
    for g in args.files:
      fs = glob.glob(g)
      if len(fs)==0: log.error(f'"{g}" does not match any files')
      for f in fs: mp4meta_export(f)
  elif args.operation=='import':
    for g in args.files:
      fs = glob.glob(g)
      if len(fs)==0: log.error(f'"{g}" does not match any files')
      for f in fs: mp4meta_import(f)
