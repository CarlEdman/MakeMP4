...
if __name__ == '__main__':
  print(sys.argv)
  parser = argparse.ArgumentParser(prog=prog, epilog=f"Written by: ${author}") # fromfile_prefix_chars='@', 
  ...
  parser.add_argument('--authorization', dest='authorization', nargs=1, help = 'obtained from themoviedb.org Access Token Auth')
  parser.add_argument('--api_key', dest='api_key', nargs=1, help = 'obtained from themoviedb.org API Key Auth')
  parser.add_argument('--accountid', dest='accountid', type=int, nargs=1, help='obtained from themoviedb.org account_id')
  ...
  ig = [pathlib.Path(d).resolve() for gd in args.paths for d in glob.iglob(gd) if pathlib.Path(d).is_file()]
  if len(ig)==0:
