...
class AdvConfig(RawConfigParser):
  ...
  def sync(self):
    if not os.path.exists(self.filename):
      with open(self.filename, "wt", encoding="utf-8") as fp:
        self.write(fp)
    elif self.modified:
  ...
  def strtoval(s):
    ...
    if s.lstrip("-").strip("0123456789") == "/":
      return Fraction(s)
    if s.lstrip("-").strip("0123456789") == ":":
      return Fraction(int(s[: s.index(":")]), int(s[s.index(":") + 1 :]))
  ...
  def set(self, opt, nval=None, section=None):
    ...
    if not section:
      raise NoSectionError("No Current Section Set")
  ...
  def has(self, opt, section=None):
    ...
    if i == "":
      return False
    if i.startswith(r"_") and i.endswith(r"_"):
      return False
    return True
