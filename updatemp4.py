...
def get_meta_local_tv(episode, ls):
  ...
  for k, v in ld[episode].items():
    ...
    elif k == 'Released' and (m := re.search(r'\b([12]\d\d\d)\b',v)): its['year']=int(m.group(0))
...
def get_meta_local_movie(ls):
  ...
  for l in ls:
    if (m:=re.fullmatch(r'(?P<year>[12]\d\d\d)\s*(?P<rating>G|PG-13|PG|R|NC-17|UR|NR|TV-14|TV-MA)\s*(?:(?P<hours>\d+)h)?\s*(?:(?P<minutes>\d+)m)?\s*(?P<format>.*)',l)):
      ...
      genres = [ genre_trans[w.strip()] for w in m.group(1).split(',') if w.strip() in genre_trans ]
      if genres:
        its['genre']=genres[0]
      else:
        its['genre']=m.group(1)
      ...
      if 'writer' in its:
        its['writer'] += f';{m.group(1)}'
      else:
        its['writer'] = m.group(1)
    elif (m := re.fullmatch(r'(\S*)\s*:*\s*(.*)',l)) and m.group(1) in beg_trans:
      comment.append(f'{beg_trans[m.group(1)]}: {m.group(2)}')
